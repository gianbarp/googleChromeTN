    ...
    itemCompare: function(type){
      ...
      for(var i in dataUpdated){
        ...
        if(dataUpdated[i].nid != dataCompare[0].nid){
          ...
          if(dataUpdated[i].media == undefined){

            dataImg = "logo.png";
          
          }else{
          
            dataImg = dataUpdated[i].media.image_thumb;
          
          }

          Noty.noties("http://tn.cdncmd.com/sites/default/files/imagecache/181x100/" + dataImg,dataUpdated[i].title,dataUpdated[i].title,store.get('timeout')*1000,dataUpdated[i].url);//.replace(/<(.)*?>/g," ")
...
/* ===========================================================================
   ...
   if(!store.get('foo')){
    ...
    store.set('foo',true); 
    Noty.noties("logo.png","TN - Todo Noticias","Mantenete informado minuto a minuto con TN.",10000,"http://tn.com.ar");
